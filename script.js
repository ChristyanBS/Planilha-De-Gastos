...
var overtimeStartDay = 24;
var overtimeEndDay = 23;

var currentYear = new Date().getFullYear();
var currentMonth = new Date().getMonth() + 1;

// =======================================================
// PONTO DE ENTRADA PRINCIPAL (STARTUP DA APLICAÇÃO)
// =======================================================
document.addEventListener('DOMContentLoaded', function() {
    auth.onAuthStateChanged(async user => {
        if (user) {
            // 1. Temos um usuário. Definimos as variáveis globais e mostramos o app.
            currentUser = user;
            ...
            // 2. Populamos os menus de seleção que não dependem de dados do banco.
            populateYearDropdown();

            // 3. Definimos os valores padrão para o mês e ano ATUAIS.
            document.getElementById('year-select').value = currentYear;
            document.getElementById('month-select').value = currentMonth;

            // 4. Carregamos as configurações iniciais e dados não periódicos (metas, investimentos).
            await loadInitialData();

            // 5. Ajustamos o tema visual.
            if (localStorage.getItem('theme') === 'dark') {
                document.documentElement.classList.add('dark');
            }
            updateThemeButton(localStorage.getItem('theme') || 'light');

            // 6. SÓ AGORA, com tudo pronto, buscamos os dados do período e atualizamos o dashboard.
            await updateDashboard();

            // 7. Finalmente, configuramos os botões e outros eventos.
            setupEventListeners();
...
async function loadInitialData() {
    ...
    [goals, investments] = await Promise.all([
    ...
    if (userSettingsDoc.exists) {
        ...
        overtimeStartDay = settings.overtimeStartDay || 24;
        overtimeEndDay = settings.overtimeEndDay || 23;
        document.getElementById('setting-overtime-start-day').value = overtimeStartDay;
        document.getElementById('setting-overtime-end-day').value = overtimeEndDay;
        expenseCategories = settings.expenseCategories || { housing: 'Moradia', food: 'Alimentação', transport: 'Transporte', health: 'Saúde', education: 'Educação', entertainment: 'Lazer', other: 'Outros' };
    } else {
        document.getElementById('header-subtitle').textContent = `${currentUser.displayName || currentUser.email}`;
        document.getElementById('setting-start-day').value = 1;
        document.getElementById('setting-overtime-start-day').value = 24;
        document.getElementById('setting-overtime-end-day').value = 23;
...
async function loadPeriodData(year, month) {
    ...
    try {
        [incomes, expenses, timeEntries] = await Promise.all([
            loadFilteredCollection('incomes'),
            loadFilteredCollection('expenses'),
            loadFilteredCollection('timeEntries')
        ]);
    } catch (error) {
        console.error("Erro Crítico ao carregar dados do período:", error);
        showToast("Falha ao carregar os dados. Verifique o console para mais detalhes.", "error");
        incomes = []; expenses = []; timeEntries = [];
    }
}
...
async function addOrUpdateItem(type, id) {
        ...
        try {
            if (id) { // Editando
                ...
                if (originalItem.installmentGroupId && originalItem.category !== itemData.category) {
                    const confirmed = await showConfirmation('Atualizar Parcelas', 'Deseja aplicar esta nova categoria para todas as parcelas futuras?', 'bg-indigo-600 hover:bg-indigo-700');
                    if (confirmed) {
                        const batch = db.batch();
                        const userDocRef = db.collection('users').doc(currentUser.uid);
                        const futureInstallmentsQuery = await userDocRef.collection('expenses').where('installmentGroupId', '==', originalItem.installmentGroupId).where('date', '>=', originalItem.date).get();
                        futureInstallmentsQuery.docs.forEach(doc => {
                            if (doc.id === id) { batch.update(doc.ref, itemData); } else { batch.update(doc.ref, { category: itemData.category }); }
                        });
                        await batch.commit();
                ...
                for (let i = 1; i <= installments; i++) {
                    ...
                    if (groupId) { newExpense.installmentGroupId = groupId; } else { newExpense.installmentGroupId = null; }
    ...
    // Lógica genérica para Renda, Metas e Investimentos (que não tem parcelamento)
    if (!isValid) {
...
async function handleDelete(type, id) {
    ...
    try {
        if (type === 'expense' && item.installmentGroupId) {
            ...
            if (confirmed) {
                const batch = db.batch();
                const futureInstallmentsQuery = await db.collection('users').doc(currentUser.uid).collection(collectionName)
...
async function saveUserSettings() {
    ...
    const newOvertimeStartDay = parseInt(document.getElementById('setting-overtime-start-day').value);
    const newOvertimeEndDay = parseInt(document.getElementById('setting-overtime-end-day').value);
    const feedbackEl = document.getElementById('settings-feedback');

    if (isNaN(newStartDay) || newStartDay < 1 || newStartDay > 28 || isNaN(newOvertimeStartDay) || newOvertimeStartDay < 1 || newOvertimeStartDay > 31 || isNaN(newOvertimeEndDay) || newOvertimeEndDay < 1 || newOvertimeEndDay > 31) {
        feedbackEl.textContent = "Por favor, insira dias válidos em todas as configurações.";
        feedbackEl.className = 'text-sm mt-2 text-center text-red-500';
        return;
    }

    payPeriodStartDay = newStartDay;
    overtimeStartDay = newOvertimeStartDay;
    overtimeEndDay = newOvertimeEndDay;
    ...
    const settings = {
        ...
        overtimeStartDay: overtimeStartDay,
        overtimeEndDay: overtimeEndDay
    };
...
// ... (Restante do arquivo)
// (Cole o resto do seu arquivo aqui, começando pelas outras funções como clearAllUserData, setupEventListeners, etc.)
// O código abaixo é o restante do arquivo, já corrigido e otimizado.
...
function setupEventListeners() {
    ...
    document.getElementById('reset-btn').addEventListener('click', async () => {
        const confirmed = await showConfirmation('Reiniciar Mês', 'Isso vai apagar os dados de renda, despesas, horas e investimentos DO PERÍODO ATUAL. Tem certeza?', 'bg-orange-500 hover:bg-orange-600');
        if (confirmed) {
            const batch = db.batch();
            [...incomes, ...expenses, ...investments, ...timeEntries].forEach(item => {
                let collectionName = '';
                if (incomes.includes(item)) collectionName = 'incomes';
                else if (expenses.includes(item)) collectionName = 'expenses';
                else if (investments.includes(item)) collectionName = 'investments';
                else if (timeEntries.includes(item)) collectionName = 'timeEntries';
                if (collectionName) batch.delete(db.collection('users').doc(currentUser.uid).collection(collectionName).doc(item.id));
            });
            await batch.commit();
            await updateDashboard();
    ...
    document.getElementById('clear-all-btn').addEventListener('click', async () => {
        const confirmed1 = await showConfirmation('ATENÇÃO MÁXIMA!', 'Isso apagará TODOS os seus dados permanentemente. Deseja continuar?');
        if (confirmed1) {
            const confirmed2 = await showConfirmation('ÚLTIMO AVISO', 'Tem certeza absoluta que deseja apagar tudo?');
            if (confirmed2) { await clearAllUserData(); }
        }
    });
...
async function clearAllUserData() {
    ...
    try {
        ...
        // Limpa os arrays locais e recarrega os dados iniciais (metas padrão, etc)
...
function setupEventListeners() {
    ...
    document.getElementById('reset-btn').addEventListener('click', async () => {
        ...
        if (confirmed) {
            const batch = db.batch();
            // Deleta itens do período atual de todas as coleções relevantes
            [...incomes, ...expenses, ...timeEntries].forEach(item => {
                let collectionName = '';
                if (incomes.includes(item)) collectionName = 'incomes';
                else if (expenses.includes(item)) collectionName = 'expenses';
                else if (timeEntries.includes(item)) collectionName = 'timeEntries';
                ...
                if (collectionName) {
                    batch.delete(db.collection('users').doc(currentUser.uid).collection(collectionName).doc(item.id));
                }
            });
            // O investimento não é deletado ao reiniciar o mês, apenas os aportes.
    ...
    document.getElementById('clear-all-btn').addEventListener('click', async () => {
        const confirmed1 = await showConfirmation('ATENÇÃO MÁXIMA!', 'Isso apagará TODOS os seus dados permanentemente, de TODOS os meses. Deseja continuar?');
        if (confirmed1) {
            const confirmed2 = await showConfirmation('ÚLTIMO AVISO', 'Tem certeza absoluta que deseja apagar tudo?');
            if (confirmed2) { await clearAllUserData(); }
        }
...
function getOvertimePeriodRange(year, month) {
    const startDay = overtimeStartDay;
    const endDay = overtimeEndDay;
    const startDate = new Date(year, month - 2, startDay);
...
function populateYearDropdown() {
    ...
    const startYear = 2024; // Começa a partir de 2024
    const endYear = currentYearDate + 10;
...
async function updateDashboard() {
    // 1. Carrega os dados do período selecionado do Firestore
    await loadPeriodData(currentYear, currentMonth);

    // 2. Calcula todos os totais com base nos dados carregados
    const totals = calculateTotals();

    // 3. Atualiza os displays de período
    const { startDate, endDate } = getPayPeriodRange(currentYear, currentMonth);
    const options = { day: '2-digit', month: '2-digit', year: 'numeric' };
    document.getElementById('date-range-display').textContent = `Exibindo período: ${startDate.toLocaleDateString('pt-BR', options)} a ${endDate.toLocaleDateString('pt-BR', options)}`;
    const { startDate: otStart, endDate: otEnd } = getOvertimePeriodRange(currentYear, currentMonth);
    const hoursPeriodEl = document.getElementById('hours-period-display');
    if (hoursPeriodEl) {
        hoursPeriodEl.textContent = `Período de apuração para horas: ${otStart.toLocaleDateString('pt-BR')} a ${otEnd.toLocaleDateString('pt-BR')}`;
    }

    // 4. Atualiza todos os cards e valores do dashboard
...
function calculateTotals() {
    // Período FINANCEIRO para rendas, despesas e investimentos
    const { startDate: financialStart, endDate: financialEnd } = getPayPeriodRange(currentYear, currentMonth);
    const financialPredicate = item => {
        const itemDate = new Date(item.date + 'T00:00:00');
        return itemDate >= financialStart && itemDate <= financialEnd;
    };
    // NOTA: Os arrays incomes, expenses já foram filtrados pelo período financeiro em loadPeriodData.
    // Mas para investimentos, que é carregado uma vez só, precisamos filtrar aqui.
    const currentInvestmentsInMonth = investments.filter(financialPredicate);

    // Período DE HORAS EXTRAS, separado do financeiro
    const { startDate: overtimeStart, endDate: overtimeEnd } = getOvertimePeriodRange(currentYear, currentMonth);
    const overtimePredicate = item => {
        const itemDate = new Date(item.date + 'T00:00:00');
        return itemDate >= overtimeStart && itemDate <= overtimeEnd;
    };
    // Filtramos o array global de timeEntries aqui, pois ele é carregado por período financeiro.
    const currentTimeEntries = timeEntries.filter(overtimePredicate);
    ...
    const homeExpenses = expenses.filter(e => e.category === 'housing').reduce((s, e) => s + e.amount, 0);
    const foodExpenses = expenses.filter(e => e.category === 'food').reduce((s, e) => s + e.amount, 0);
    const transportExpenses = expenses.filter(e => e.category === 'transport').reduce((s, e) => s + e.amount, 0);
    const healthExpenses = expenses.filter(e => e.category === 'health').reduce((s, e) => s + e.amount, 0);
    const educationExpenses = expenses.filter(e => e.category === 'education').reduce((s, e) => s + e.amount, 0);
    const entertainmentExpenses = expenses.filter(e => e.category === 'entertainment').reduce((s, e) => s + e.amount, 0);
...
function updateIncomeTable() {
    ...
    const currentData = [...incomes].sort((a, b) => new Date(b.date) - new Date(a.date));
...
function updateExpensesTable() {
    const currentData = [...expenses].sort((a, b) => new Date(b.date) - new Date(a.date));
...
async function toggleExpenseStatus(id) {
    ...
    if (expense) {
        ...
        try {
            ...
            expense.isPaid = !expense.isPaid; // Reverte em caso de erro
        }
    }
}
...
function updateHoursTable() {
    const { startDate, endDate } = getOvertimePeriodRange(currentYear, currentMonth);
    const predicate = item => {
        const itemDate = new Date(item.date + 'T00:00:00');
        return itemDate >= startDate && itemDate <= endDate;
    };
...
async function addHourEntry() {
    ...
    try {
        if (editingId) {
            ...
            await db.collection('users').doc(currentUser.uid).collection('timeEntries').add(entryData);
        }
        resetHourForm();
        await updateDashboard();
...
// =======================================================
// LÓGICA DE INSTALAÇÃO DO PWA E NOTIFICAÇÕES
// =======================================================
...
// FUNÇÕES DE NOTIFICAÇÃO (TOAST E CONFIRMAÇÃO)
// =======================================================
...
async function adicionarSenhaNaConta(novaSenha) {
    ...
    const temProvedorSenha = user.providerData.some((provider) => provider.providerId === 'password');
    try {
        if (temProvedorSenha) {
